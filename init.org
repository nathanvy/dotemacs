...
** Infrastructure
   First set the package archives.  I use MELPA as well as the Org-specific repository, so we'll set those.  I prefer use-package to el-get or straight.el so that's what I happen to use.
...
** Editing
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package company
       :hook (init-mode-hook . global-company-mode)
       :mode "company-mode")
